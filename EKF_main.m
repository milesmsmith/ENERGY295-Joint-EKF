...
for k = 2:1:N 
    ...
    %----predict step----
    mu_predict = Akm1*mu(:,k-1) + Bkm1*I(k-1); % mu_k|k-1
%     mu_predict(1) = saturate(mu_predict(1),0,100);
%     mu_predict(2) = relu(mu_predict(2));
    S_predict = Akm1*S(:,:,k-1)*Akm1' + Q; %S_k|k-1
    % evaluate jacobian of C
    if mu_predict(1) < 100 && mu_predict(1) > 0
        dVocdSOC = polyval(p_dOCVdSOC_map,mu_predict(1));
    else
        dVocdSOC = 0;
    end
    Ck = [dVocdSOC, -I(k), -1, -1]; %Evaluate Ck at mu_k|k-1
    %----update step----
    K = S_predict*Ck'*inv(Ck*S_predict*Ck' + R); %Kalman Gain
    mu(:,k) = mu_predict + K*(V(k) - (polyval(p_SOC_OCV_map,saturate(mu_predict(1),0,100)) - mu_predict(3) - mu_predict(4) - I(k)*mu_predict(2))); %Evaluate at mu_k|k-1
%     mu(1,k) = saturate(mu(1,k),0,100);
%     mu(2,k) = relu(mu(2,k));
    S(:,:,k) = S_predict - K*Ck*S_predict; %Evaluate at S_k|k-1,Ck
    Vb(k) = polyval(p_SOC_OCV_map,saturate(mu(1,k),0,100)) - mu(3,k) - mu(4,k) - I(k)*mu(2,k); % yk = g(mu_k|k)
end
