%% Finding R0-C2 using graphical and GA techniques...
...
%% Finding Q and R using GA
load('ECM_params.mat');

duration = 4000;
t = t(start_idx:start_idx + duration);
V_expt = V_expt(start_idx:start_idx + duration);
I_expt = I_expt(start_idx:start_idx + duration);
SOC_CC = SOC_CC(start_idx:start_idx + duration);

R0_mean = mean([R0_dischg;R0_chg]);
SOC0 = 0.9;
x_t0 = [SOC0;0;0;R0_mean];
P_t0 = diag([0.1,0.001,0.001,0.1]);
R = 8.432e-4;
...
[ga_params, ga_loss] = ga(@(ga_params)objective_function_EKF_SOC_V1_V2_R0...
    (ga_params,Q,R,...
        SOC_CC,dt,V_expt,I_expt,x_t0,P_t0,...
        R1_chg,R1_dischg,C1_chg,C1_dischg,R2_chg,R2_dischg,C2_chg,C2_dischg,...
        soc_chg,soc_dischg,Qn,Voc_vs_SOC,deltaSOC,Adaptive,W),num_params,[],[],[],[],lb,ub,[],options);
Q = diag([diag(Q); exp(ga_params)])
R = R
...
function loss = objective_function_EKF_SOC_V1_V2_R0(ga_params,Q,R,...
        SOC_Coulomb_Counting,dt,V_expt,I_expt,x_t,P_t,...
        R1_chg,R1_dischg,C1_chg,C1_dischg,R2_chg,R2_dischg,C2_chg,C2_dischg,...
        soc_chg,soc_dischg,Qn,Voc_vs_SOC,deltaSOC,Adaptive,W)
    Q = diag([diag(Q); exp(ga_params)]);
    [x_t,Vb,L] = EKF(dt,V_expt,I_expt,x_t,P_t,Q,R,...
        R1_chg,R1_dischg,C1_chg,C1_dischg,R2_chg,R2_dischg,C2_chg,C2_dischg,...
        soc_chg,soc_dischg,Qn,Voc_vs_SOC,deltaSOC,Adaptive,W);
    loss = calc_percent_rmse(SOC_Coulomb_Counting,x_t(1,:));
    disp(['Loss: ' num2str(loss)]);
    disp(['GA_param: ' num2str(exp(ga_params))]);
end

function [x_t,Vb,L] = EKF(dt,V_expt,I_expt,x_t,P_t,Q,R,...
        R1_chg,R1_dischg,C1_chg,C1_dischg,R2_chg,R2_dischg,C2_chg,C2_dischg,...
        soc_chg,soc_dischg,Qn,Voc_vs_SOC,deltaSOC,Adaptive,W)
    N = length(V_expt);
    for i = 2:N
